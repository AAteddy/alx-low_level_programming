...
/**
  * verify_elf - verifies if the file is an ELF file
  * @elf: a pointer to an ELF struct
  *
  * Desc: If not an ELF file - exit code 98
  */
void verify_elf(unsigned char *elf)
{
	int i;
...
/**
  * magic_elf - a function that prints ELF magic number
  * @elf: a pointer to a elf struct
  */
void magic_elf(unsigned char *elf)
{
	int index;
